...
public class Serial implements SerialPortEventListener {
  ...
  public Serial(String port) throws Exception {// throws SerialException {
    this(port, 9600, 'N', 8, 1, true, true);
  }
/*
...
*/
  protected Serial(String iname, int irate, char iparity, int idatabits, float istopbits, boolean setRTS, boolean setDTR) throws Exception/* throws SerialException*/ {
    ...
    try {
      ...
      if (!res) {
        System.err.println("Error while setting serial port parameters");//: " + irate, iparity, idatabits, istopbits{0} {1} {2} {3}"),
      }
    ...
    if (port == null) {
      throw new Exception("Serial port " + iname + " not found. Did you select the right one from the Tools > Serial Port menu?");
    }
  }
  ...
  private static void errorMessage(String where, Throwable e) {
    System.err.println("Error inside Serial.{0}()");//), where));
    e.printStackTrace();
  }
}
